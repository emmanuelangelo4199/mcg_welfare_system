...
from django.utils import timezone
from .models import Member, MemberRegularisation
...
def member_registration_view(request):
    ...
    if len(selected_organisations) != len(set(selected_organisation_ids)):
        valid_organisation_ids = [org_id for org_id in selected_organisation_ids if org_id.isdigit()]
    selected_organisations = list(Organisation.objects.filter(id__in=valid_organisation_ids))
    if len(valid_organisation_ids) != len(selected_organisation_ids) or len(selected_organisations) != len(set(valid_organisation_ids)):
